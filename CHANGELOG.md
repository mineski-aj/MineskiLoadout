...
- **Floorplan text labels** — a "🔤 Add Text" tool for dropping freeform, editable labels onto the floorplan image (e.g. to name specific rooms/blocks). Drag to move, drag the corner handle to resize (scales the font size proportionally), click to edit the text, hover for a delete button. Syncs per-sheet through the same save pipeline as everything else; existing floorplans without any labels are unaffected.
