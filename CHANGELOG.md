...
- **Floorplan text labels** — a "🔤 Add Text" tool for dropping freeform, editable labels onto the floorplan image (e.g. to name specific rooms/blocks). Centered both ways by default, drag to move, drag the corner handle to resize (scales the font size proportionally), click to edit the text, hover for a delete button. Text **auto-shrinks to fit** the box as you type (and grows back once it's short enough again), so a long label wraps and scales down instead of getting clipped. Syncs per-sheet through the same save pipeline as everything else; existing floorplans without any labels are unaffected.
